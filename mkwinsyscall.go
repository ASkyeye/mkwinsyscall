...
// - manual module handle resolution (github.com/atsika/winsolve)
// - manual function address resolution (github.com/atsika/winsolve)
// - API hashing (sdbm)
...
func (f *Fn) DLLFuncNameHash() string {
	if f.dllfuncname == "" {
		return fmt.Sprintf("uint32(%#2x)", hash(f.Name))
	}
	return fmt.Sprintf("uint32(%#2x)", hash(f.dllfuncname))
}
	...
	funcMap := template.FuncMap{
		...
		"newlazydll": func(dll string) string {
			...
			default:
				...
				return "winsolve.NewDLL(" + hashed + ") // " + arg
			}
		},
	}
...
{{define "funcnames"}}{{range .DLLFuncNames}}	proc{{.DLLFuncName}} = winsolve.NewProc(mod{{.DLLVar}}, {{.DLLFuncNameHash}}) // {{.DLLFuncName}}
{{end}}{{end}}
